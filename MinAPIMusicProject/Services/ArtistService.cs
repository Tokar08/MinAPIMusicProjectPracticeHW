...
public class ArtistService : IArtistService
{
    ...
    {
        var artistFromDb = _context.Add((object)_mapper.Map<Artist>(artist));
    ...
    {
        var artist = await _context.Artists.FindAsync(new object[] { id }, cancellationToken);

        if (artist == null) throw new ArgumentNullException(nameof(artist) + " is null");
    ...
    public async Task<TrackDTO> AddTrack(int artistId, AddTrackDTO track, CancellationToken cancellationToken = default)
    {
        ...
        if (genre == null) throw new ArgumentNullException(nameof(genre), " is null");

        trackToAdd.Artist = artist;
        trackToAdd.Genre = genre;

        _context.Add((object)trackToAdd);
        await _context.SaveChangesAsync(cancellationToken);
